    ...
    return (
      <nav className="flex flex-col gap-6">
        {navigationData.map((item) => {
          const itemUrl = buildLocationUrl(item.href, selectedLocation.code, item.useLocation);
          return (
            ...
            {item.dropMenu && openDropdown === item.name && (
              <div className="flex flex-col my-2 gap-3 pl-4 border-l-2 border-[#0D72B6]">
                {item.dropMenu.map((subitem) => (
                  <Link
                    key={subitem.name}
                    href={buildLocationUrl(subitem.href, selectedLocation.code, subitem.useLocation)}
                    onClick={props.onItemClick}
