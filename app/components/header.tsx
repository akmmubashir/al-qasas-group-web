...
const Header = () => {
  ...
  return (
    <>
      <div
        className={`fixed w-full left-0 top-0 z-40 transition-transform duration-300 ${visible ? "translate-y-0" : "-translate-y-full"} ${scrolled ? "bg-white shadow-2xl items-end" : "bg-white"}`}
      >
        <div
          className={`p-[20px_80px] max-xl:p-[20px_40px] max-md:p-[16px_20px]  flex items-center justify-between`}
        >
          <Logo color={scrolled ? "black" : "black"} />
